---
description: The "Debug & Rescue" Mission
---
# The "Debug & Rescue" Mission

← Back to Index

Role:ML Engineer / ScientistGoal:Diagnose why amassive 128-GPU training run crashed
 overnight and resume it without losing progress.


Phase 1: The Crash Report ("Forensic Analysis")
Context:The scientist wakes up to find their jobmarked as `FAILED`. They need to know why
 (Hardware failure? Code bug? OOM?) without reading 10,000 lines of logs.
...
Command:Analyze the failure.Action:GREMLIN parsesthe stderr logs, correlates with node
 heartbeats (`Hypernexus`), and identifies the root cause (e.g., OOM during validation loop).
...
Context:They need to restart the job, but with afix. They don't want to restart from Epoch 0
 (wasting $5k in compute). They need to resume from the last valid checkpoint.
...
Command:Apply the fix and auto-resume.Action:GREMLINpatches the config and finds the
 latest valid checkpoint in Atlas.
...
Context:The job is running again. The scientist doesn'twant to wake up to another crash.
 GREMLIN can monitor the run and alert on anomalies.
...
Command:Enable monitoring.Action:GREMLIN watchesthe W&B metrics stream and performs
 anomaly detection on loss curves, GPU utilization, and memory pressure.
...
GREMLIN maintains a knowledge base of Tempus-specific failure modes:
 ...
 `NCCL timeout          GPU sync timeout         Increase `NCCL_TIMEOUT` or reduce
  exceeded`              (TCP/IP latency)         sync frequency

 `CUDA OOM`             Batch too large for      Reduce batch size or enable gradient
                           VRAM                     checkpointing

 `Ray actor died`       Node preemption (Spot)   Auto-resume from checkpoint

 `Connection refused:   Network partition        Retry with backoff
  Atlas`
`FileNotFoundError:    Stale GCS cache          Clear gcsfuse cache, re-mount
 checkpoint`
