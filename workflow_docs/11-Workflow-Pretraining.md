# Workflow: The Unimodal Pretraining Run

[← Back to Index](00-Index.md)

**Role:** NLP Scientist / Research Platform Engineer
**Goal:** Train a foundation model on Tempus clinical text data.
**Key Challenge:** Scale. This is not a fine-tuning job; it requires terabytes of data, weeks of compute, and the ability to survive hardware failures.
...
**Context:** The raw dataset is 10TB of text. It cannot fit in memory. It must be tokenized, chunked, and sharded across 100+ files for distributed loading.
...
### 👾 The GREMLIN Way
**Command:** Prepare the corpus.
**Action:** GREMLIN spins up a serverless Ray Data cluster, optimizes the shard size for the target batch size, and streams the result to a pinned GCS bucket.
...
**Context:** Training will take 2 weeks. Using "On-Demand" GPUs costs $50,000. Using "Spot Instances" (preemptible) costs $15,000, but nodes will randomly die.

### 🛑 The Old Way
*   Use On-Demand nodes (Expensive).
*   Or, use Spot nodes and wake up at 3 AM when the cluster crashes to manually restart from checkpoint.

### 👾 The GREMLIN Way
**Command:** Train with resilience.
**Action:** GREMLIN configures `Ray Connect` to handle "Elastic Training." If a node dies, GREMLIN automatically detects the loss, pauses the master worker, waits for a replacement node, and resumes training—**without human intervention**.
...
**Context:** End-to-end unstructured text processing for embedding generation and model pretraining.
...
| **Chunking** | Segment into model-compatible windows (2K tokens, 50% overlap) | 1-2 hours |
| **Embedding** | Generate vectors via Triton Inference Server | See below |
| **Redaction** | Drop attachments after anchor dates (LOT, diagnosis, metastasis) | Event-based filter |

### Embedding Generation Scale

| Model | Embedding Dim | Chunk Size | GPUs | Throughput | Runtime |
| :--- | :--- | :--- | :--- | :--- | :--- |
| BGE-small | 384 | 512 | 16 | 32K req/s | 15 hours |
| ModernBERT | 768 | 8192 | 64 | 4K req/s | 2 days |
| MedGemma-27B | 5376 | 8192 | 108 | 500 req/s | 12+ hours |

### 👾 The GREMLIN Way

**Command:** Run the full pipeline.
**Action:** GREMLIN orchestrates all stages, tracking progress and handling failures.
...
[LOG] Stages:
  1. Clean (text-corpus-processing)     ... COMPLETE
  2. Chunk (pdf-attachments-chunker)    ... COMPLETE
  3. Stream (Lightning StreamingDataset) ... RUNNING
  4. Embed (MedGemma-27B via Triton)     ... PENDING
...
**Context:** Fine-tuning MedGemma-27B on Tempus clinical notes using FSDP/DeepSpeed.
...
**Context:** The pretraining job runs for days or weeks. The scientist needs to know if the loss curve is healthy or if it's diverging without staring at dashboards all weekend.
...
**Action:** GREMLIN queries the W&B API for run metrics and performs anomaly detection on the loss curve.
...
| Loss | 2.14 | 📉 Decreasing normally |
| MFU | 48% | ✅ Excellent |
| Tokens Seen | 2.0B | On track |
| GPU Utilization | 94% avg | ✅ Healthy |
